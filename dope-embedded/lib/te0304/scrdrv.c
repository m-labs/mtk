...
/*
 * Current overlay position, initialized such that overlay is
 * disabled. The point (x1, y1) is the top-left corner and the
 * point (x2, y2) is the bottom-right corner of the overlay area.
 * Set the overlay position via function 'scrdrv_overlay()'.
 */
static long ovl_x1 = 0, ovl_y1 = 0, ovl_x2 = -1, ovl_y2 = -1;
...
/**
 * Return true if mouse cursor is above the overlay area
 */
static inline int mouse_intersects_overlay(void)
{
	return (curr_mx >= ovl_x1 && curr_mx + 16 <= ovl_x2
	     && curr_my >= ovl_y1 && curr_my + 16 <= ovl_y2);
}
...
/**
 * Overlay-enhanced version of update_area function
 *
 * This version of the function omits all pixels within the overlay
 * area. It intersects the overlay area with the update request and
 * successively updates the area around the overlay (top, bottom,
 * left, and right) by calling the original 'update_area' function
 * for each part.
 */
static void update_area_cut_out_overlay(long x1, long y1, long x2, long y2)
{
	/* opdate area above the overlay */
	if (y1 < ovl_y1) {
		long cut_y = MIN(ovl_y1 - 1, y2);
		update_area(x1, y1, x2, cut_y);
		y1 = cut_y + 1;
	}

	/* opdate area below the overlay */
	if (y2 > ovl_y2) {
		long cut_y = MAX(ovl_y2 + 1, y1);
		update_area(x1, cut_y, x2, y2);
		y2 = cut_y - 1;
	}

	/* update area left of the overlay */
	if (x1 < ovl_x1)
		update_area(x1, y1, MIN(ovl_x1 - 1, x2), y2);

	/* update area right of the overlay */
	if (x2 > ovl_x2)
		update_area(MAX(ovl_x2 + 1, x1), y1, x2, y2);
}


/**
 * Set current overlay position
 *
 * Once defined by this function, the specified overlay screen area will
 * not be touched by DOpE such that another process can write pixels to
 * the overlay screen region.
 *
 * \param x1, y1  top-left corner of the overlay
 * \param x2, y2  bottom-right corner of the overlay
 */
void scrdrv_overlay(long x1, long y1, long x2, long y2)
{
	int old_x1 = ovl_x1, old_y1 = ovl_y1, old_x2 = ovl_x2, old_y2 = ovl_y2;
	ovl_x1 = x1, ovl_y1 = y1, ovl_x2 = x2, ovl_y2 = y2;

	if (scr_adr)
		update_area_cut_out_overlay(old_x1, old_y1, old_x2, old_y2);
}
...
static void set_mouse_pos(long mx, long my)
{
	...
	curr_mx = mx;
	curr_my = my;
	update_area_cut_out_overlay(curr_mx, curr_my, curr_mx + 16, curr_my + 16);
	update_area_cut_out_overlay(old_mx,  old_my,  old_mx  + 16, old_my  + 16);
}
