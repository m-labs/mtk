...
//function returns pointer to XIntc InterruptController structure
u32 get_InterruptController(void)
{
	return (u32)&InterruptController;
}
...
{
	int scancode;
	static u32 old_keystate = 0;  //Modified by Lovro -- 26.11.2008
	...
	//modified by Lovro -- 26.11.2008
	//This code works fine (robust) but doesn't allow shift + key...
	//We don't care about the release event so when the key is pressed, the
	//release event is automatically enqued after the press event.
	//This allows us to hold down a key and get multiple characters in terminal.
	//With original code it led to strange behaviour - it was impossible to 
	//select another window after press and hold a key.
	/*if (!(keystate & KEYBD_TYPE_MASK))
	{
		enqueue_event(EVENT_PRESS, scancode & KEYBD_CODE_MASK, 0, 0);
		enqueue_event(EVENT_RELEASE, scancode & KEYBD_CODE_MASK, 0, 0);
	}*/
	
	/* key released */
	if(old_keystate != keystate) //press
	{
		// key released
		if (keystate & KEYBD_TYPE_MASK)
			enqueue_event(EVENT_RELEASE, scancode & KEYBD_CODE_MASK, 0, 0);
	...
	{	
		//key has been pressed and is beeing hold down, so we need to release
		//and press again to interpret this as new character
		enqueue_event(EVENT_RELEASE, scancode & KEYBD_CODE_MASK, 0, 0);
		enqueue_event(EVENT_PRESS, scancode & KEYBD_CODE_MASK, 0, 0);
	}
