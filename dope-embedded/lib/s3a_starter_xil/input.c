...
int init_input(struct dope_services *d)
{
	...
	/* Register the PS/2 Interrupt Handler at Xilkernel */
	Status = register_int_handler(XPAR_XPS_INTC_0_PLB_PS2_CONTROLLER_0_IP2INTC_IRPT_INTR, PS2_InterruptHandler, NULL);

	/* Enable the PS/2 interrupt */
	enable_interrupt(XPAR_XPS_INTC_0_PLB_PS2_CONTROLLER_0_IP2INTC_IRPT_INTR);
